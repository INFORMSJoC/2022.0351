...
cte_config['11'] =[10, 8, 12, 6, 14, 10, 10]
cte_config['12'] = [5, 6, 4, 5, 5, 5]
cte_config['13'] = [0.2, 0.18, 0.16, 0.14, 0.12, 0.1 ]
cte_config['14'] = [0.2, 0.22, 0.18, 0.22, 0.18, 0.2 ]



cte_config['gamma'] = 0.3

cte_config['round'] =  1# default = 1
...
if cte_config['em_kde'] == 0:
    ...
    for i in range(cte_config['round']):
        ...
        if cte_config['simulation'] == 3:
            pp = STPPS3(cte_config['lam'],cte_config['alpha'],cte_config['beta'],cte_config['gamma'],cte_config['sigx'],cte_config['sigy'], cte_config)
            Ini = pp.InitialGenerate(T=[0, 10], S=[[-5, 5], [-5, 5]])
            Ini1 = Ini
            All = [Ini]
            Index = []
            ...
            ct = 0
            while True:
                Ini1, PC = pp.OffspringPropagation(Ini,decay, cte_config['11'], cte_config['12'], cte_config['13'], cte_config['14'])
                if Ini1.shape == (0,3):
                    break
                else:
                    All.append(Ini1)
        ...
        def Index2P_Hier(data,index,allinfer):
            P_list = []
            shape = data.shape[0]
            background = allinfer[0]
            for i in range(len(allinfer)):
        ...
        if cte_config['cte_']:
            ...
            # print(result2.fun)
            cte_i = []
            P_list = Index2P_Hier(data,Index_Infer,All_Infer)
            for item in P_list:     
                P_mat = item
                xa = cte_config['initial_guess']
                result2 = minimize(fun = loglikihood, x0 = xa, method = 'Nelder-Mead', bounds=bounds)
                cte_i.append(result2.x)
            cte_all.append(cte_i)
            # print(result2)
